function varargout = plot_google_map_test(varargin)
...
maptypeStr2 = ['&maptype=satellite' ];
filename2 = 'tmp2.jpg';

url1 = [preamble location zoomStr sizeStr maptypeStr format markers labelsStr languageStr sensor keyStr];
url2 = [preamble location zoomStr sizeStr maptypeStr2 format markers labelsStr languageStr sensor keyStr];


    
% Get the image
try
    urlwrite(url1,filename);
...
imag2 = zeros(height2,width2,3);
for idx = 1:3
    imag2(:,:,idx) = reshape(M2color(M2(:)+1+(idx-1)*size(M2color,1)),height2,width2);
end
...
uniImag2 = zeros(uniHeight,uniWidth,3);
...
uniImag2 =  myTurboInterp2(lonMesh,latMesh,imag2,uniLonMesh,uniLatMesh);
